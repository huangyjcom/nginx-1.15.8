...
static ngx_command_t ngx_http_hello_world_commands[] = {
    ...
    ngx_null_command  //固定加一个ngx_null_command作为结尾
};
...
static ngx_int_t ngx_http_hello_world_handler(ngx_http_request_t* r) {
    ...
    b->memory = 1;
	content_length += sizeof("hello_world, ") - 1;
    ...
    b->memory = 1;
	content_length += hlcf->output_words.len;
	...
	ngx_uint_t i = 0;
	ngx_table_elt_t *header = r->headers_in.headers.part.elts;
    ...
    r->headers_out.content_length_n = content_length;//hlcf->output_words.len + sizeof("hello_world, ") - 1;
...
static char* ngx_http_hello_world(ngx_conf_t* cf, ngx_command_t* cmd, void* conf) {
    ...
    clcf->handler = ngx_http_hello_world_handler; // handler在ngx_http_update_location_config函数中赋值给r->content_handler，在ngx_http_core_content_phase产生内容阶段的函数中，就会执行这个回调函数
