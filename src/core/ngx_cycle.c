...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    if (ngx_conf_param(&conf) != NGX_CONF_OK) {//解析nginx命令行参数’-g’加入的配置
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return NULL;
    }

    if (ngx_conf_parse(&conf, &cycle->conf_file) != NGX_CONF_OK) {//解析nginx配置文件
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return NULL;
    }
    ...
    if (ngx_open_listening_sockets(cycle) != NGX_OK) {//打开监听端口
        goto failed;
    }
