...
void
ngx_log_error_core(ngx_uint_t level, char *file, int line, ngx_log_t *log, ngx_err_t err,
    const char *fmt, ...)
...
#endif
{
	...
	char tmp_file[25];
	memset(tmp_file, 32, 24);
	tmp_file[24] = 0;
	int len = strlen(src_file);
	if (len < 24) {
		memcpy(tmp_file + 24 - len, src_file, len);
	} else {
		memcpy(tmp_file, src_file, 24);
	}
	

    p = ngx_slprintf(p, last, " %s|%5d| [%V] ", tmp_file, line, &err_levels[level]);
...
void ngx_cdecl
ngx_log_error(ngx_uint_t level, char *file, int line, ngx_log_t *log, ngx_err_t err,
    const char *fmt, ...)
{
    va_list  args;
    ...
    if (log->log_level >= level) {
        va_start(args, fmt);
        ngx_log_error_core(level, file, line, log, err, fmt, args);
        va_end(args);
    }
}
...
void ngx_cdecl
ngx_log_debug_core(char *file, int line, ngx_log_t *log, ngx_err_t err, const char *fmt, ...)
{
    va_list  args;

    va_start(args, fmt);
    ngx_log_error_core(NGX_LOG_DEBUG, file, line, log, err, fmt, args);
    va_end(args);
}
