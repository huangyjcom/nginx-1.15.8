...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    for ( ;; ) {
        rc = ngx_conf_read_token(cf);//词法分析
        ...
        rc = ngx_conf_handler(cf, rc);//指令解析方式2--nginx内建的指令解析
...
ngx_conf_handler(ngx_conf_t *cf, ngx_int_t last)
{
    ...
    for (i = 0; cf->cycle->modules[i]; i++) {//其实nginx就是去遍历所有的模块
        ...
        for ( /* void */ ; cmd->name.len; cmd++) {
            ...
            if (!(cmd->type & NGX_CONF_ANY)) {//指令参数个数必须正确
            ...
            if (cmd->type & NGX_DIRECT_CONF) {//NGX_DIRECT_CONF常量单纯用来指定配置存储区的寻址方法，只用于core模块
            	...
            	//NGX_MAIN_CONF常量有两重含义，其一是指定指令的使用上下文是main（其实还是指core模块），其二是指定配置存储区的寻址方法
                conf = &(((void **) cf->ctx)[cf->cycle->modules[i]->index]);

            } else if (cf->ctx) {
            	//除开core模块，其他类型的模块都会使用第三种配置寻址方式，也就是根据cmd->conf的值从cf->ctx中取出对应的配置。
            	//举http模块为例，cf->conf的可选值是NGX_HTTP_MAIN_CONF_OFFSET、NGX_HTTP_SRV_CONF_OFFSET、NGX_HTTP_LOC_CONF_OFFSET，
            ...
            rv = cmd->set(cf, cmd, conf);//执行指令解析回调函数
...
char * //读取整数类型(有符号整数ngx_int_t)的参数
...
char * //读取的参数值是2个，一个是buf的个数，一个是buf的大小。例如： output_buffers 1 128k
...
char * //读取枚举类型的参数，将其转换成整数ngx_uint_t类型, eg: gzip_static
...
char * //读取参数的值，并将这些参数的值以bit位的形式存储。例如：HttpDavModule模块的dav_methods指令
