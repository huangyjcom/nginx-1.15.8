...
static ngx_int_t //注册到ngx_http_top_header_filter，把自己的filter函数ngx_http_not_modified_header_filter添加到链表头
ngx_http_not_modified_filter_init(ngx_conf_t *cf)
{
    ngx_http_next_header_filter = ngx_http_top_header_filter; //ngx_http_next_header_filter是此模块的static变量，保存上一个filter模块的回调函数
    ngx_http_top_header_filter = ngx_http_not_modified_header_filter;//ngx_http_top_header_filter链表表执行此modulte的filter回调函数
