...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    while (ph[r->phase_handler].checker) {
		ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "core_run: phase_handler=%ui checker=%p", r->phase_handler, ph[r->phase_handler].checker);

        rc = ph[r->phase_handler].checker(r, &ph[r->phase_handler]);
		//rc返回值: NGX_OK -- 表示该请求已经处理结束   NGX_AGAIN -- 表示还将执行另一个phase的处理
        if (rc == NGX_OK) {
            return;
        }
    }
...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "generic phase: %ui handler=%p", r->phase_handler, ph->handler);
...
ngx_http_core_rewrite_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "rewrite phase: %ui  handler=%p", r->phase_handler, ph->handler);
...
ngx_http_core_access_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "access phase: %ui handler=%p", r->phase_handler, ph->handler);
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    if (r->content_handler) {//模块可以设置独立的content handler，
    	//eg: ngx_http_proxy_module的proxy_pass指令会设置一个名为ngx_http_proxy_handler的content handler
    	//eg: ngx_http_hello_world_handler模块的ngx_http_hello_world_handler回调函数
		ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "content phase: %ui  content_handler=%p handler=%p", r->phase_handler, r->content_handler, ph->handler);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "content phase: %ui  handler=%p", r->phase_handler, ph->handler);

    rc = ph->handler(r);//默认情况下，nginx会在CONTENT阶段的handler链挂上index模块，静态文件处理模块等的handler
...
ngx_http_send_header(ngx_http_request_t *r)
{
    ...
    return ngx_http_top_header_filter(r);//指向ngx_http_not_modified_filter_module的ngx_http_not_modified_header_filter函数(这个是最晚注册的filter模块，但最早执行)
}
