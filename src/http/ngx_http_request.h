...
struct ngx_http_request_s {
    ...
    */ 
    ...
    ngx_http_request_t               *main;//一定是原始请求
    ...
    ngx_http_handler_pt               content_handler;//表示NGX_HTTP_CONTENT_PHASE阶段提供给http模块处理请求的一种方式，它指向http模块实现的请求处理方法
    ngx_uint_t                        access_code;//在NGX_HTTP_ACCESS_PHASE节点需要判断请求是否具有访问权限时，通过access_code来传递http模块的handler回调方法的返回值，如果为0表示具备权限。否则不具备。 
	/*这是一个ngx_http_variable_value_t的数组，保存了所有缓存的变量值。
	  注意，r->variables和cmcf->variables是一一对应的关系，有点像key-value的那种对应关系，
	  即cmcf->variables上的所有变量，其变量值都对应缓存在了r->variables上，
	  v->no_cacheable标志位决定了是调用变量的get_handler获取变量的值还是直接从缓存数组r->variables上获取变量值
	*/
	...
	/*
	r->limit_rate，这个表示当前的request的发送限制速率，这个也是在nginx.conf中配置的，
	而一般就是通过这个值来设置c->write->delayed的。
	也就是说如果发送速率大于这个limit了的话，就设置delayed，然后这边的request就会延迟发送
	*/
    size_t                            limit_rate;
    ...
    ngx_http_cleanup_t               *cleanup;//在这个请求中如果打开了某些资源，并需要在请求结束时释放，那么需要把定义的释放资源的方法添加到这个成员
	//引用计数一般都作用于这个请求的原始请求上 
    //引用计数，每当派生出子请求时，原始请求的count成员都会加一
    unsigned                          count:16;
    unsigned                          subrequests:8;//subrequests被设置为51，表示一个请求最多可以发起50个子请求.引用一次值就减去一
    unsigned                          blocked:8;//阻塞标志位，目前仅由aio使用

    unsigned                          aio:1;//标志位：为1表示蛋清请求正在使用异步IO 
    ...
    unsigned                          invalid_header:1;//标示header是否有效，不正常的结束视为无效

    unsigned                          add_uri_to_alias:1;
    unsigned                          valid_location:1;
    unsigned                          valid_unparsed_uri:1;
    unsigned                          uri_changed:1;//标志位：为1时表示URL发生过rewrite重写
    unsigned                          uri_changes:4;//表示使用rewrite重写URL的次数(最多改写10次)
    ...
    unsigned                          header_sent:1;//标志位：为1表示发生给客户端的http响应头已经发送
    ...
    /* used to parse HTTP headers 解析消息头时用到的一些变量*/
