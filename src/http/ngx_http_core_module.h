...
typedef enum {
    ...
    NGX_HTTP_SERVER_REWRITE_PHASE,  //server级别的uri重写阶段，也就是该阶段执行处于server块内，location块外的重写指令
    ...
    NGX_HTTP_PRECONTENT_PHASE,		//NGX_HTTP_TRY_FILES_PHASE ??? try_files指令的处理阶段，如果没有配置try_files指令，则该阶段被跳过
...
struct ngx_http_phase_handler_s {
    ngx_http_phase_handler_pt  checker; //相同阶段的节点具有相同的checker函数
...
typedef struct {
	...
	//这是一个hash表，保存了cmcf->variables_keys中所有变量（除去那些带有NGX_HTTP_VAR_NOHASH标志的变量）
    ngx_hash_t                 variables_hash;
	
	//这是一个array数组，收集了用户已经使用的所有变量，这些变量里可能有的是非法的变量，即未“声明”过就使用，这种合法性判断会在解析完配置文件后进行
    ngx_array_t                variables;         /* ngx_http_variable_t */ //在ngx_http_get_variable_index函数添加变量

	ngx_array_t                prefix_variables;  /* ngx_http_variable_t */ //在ngx_http_add_prefix_variable函数添加变量
	...
	//这是一个临时的容器，保存了所有“声明”过的变量，配置文件解析完成后将会释放，而该容器中所有变量会被hash的变量和未被hash的变量分类保存，被hash过的变量里面，又细分为索引的变量和未进行索引的变量，而在索引过的变量里面，又细分为需要缓存的变量和未不需要缓存的变量
