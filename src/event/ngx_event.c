...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    for (m = 0; cycle->modules[m]; m++) {/* 初始化事件模型 */
    ...
    ls = cycle->listening.elts;
    for (i = 0; i < cycle->listening.nelts; i++) {/* 为每个监听套接字分配一个连接结构 */
        ...
        if (ngx_event_flags & NGX_USE_IOCP_EVENT) {//iocp是Windows NT操作系统的一种高效IO模型，对应于Linux中的epoll和FreeBSD中的kqueue
        ...
        rev->handler = (c->type == SOCK_STREAM) ? ngx_event_accept
                                                : ngx_event_recvmsg;//设置端口的accept函数
        ...
        if (ngx_use_accept_mutex) {/* 如果使用accept锁的话，要在后面抢到锁才能将监听句柄挂载上事件处理模型上 */
            continue;
        }
